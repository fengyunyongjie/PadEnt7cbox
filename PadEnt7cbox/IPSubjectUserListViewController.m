...
@interface IPSubjectUserListViewController ()
...
@implementation IPSubjectUserListViewController
...
-(void)selectAll:(id)sender
{
    ...
    if ([self.navigationItem.rightBarButtonItem.title isEqualToString:@"全选"])
    {
        ...
        for (int i = 0; i < self.listArray.count; i++) {
            IPUserItem *item = [self.listArray objectAtIndex:i];
            item.checked = YES;
        }
        ...
        if (self.dataDic) {
            ...
            for (int i = 0; i < self.sectionArray.count; i++) {
                ...
                for (int j = 0; j < memberArray.count; j++) {
                    ...
                    IPUserItem *item = [[IPUserItem alloc] init];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UIImageView *imageV2 = [[UIImageView alloc] initWithFrame:CGRectMake(-10, 29, 320, 1)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.sectionArray) {
        ...
        if (dic)
        {
            l.text=[dic objectForKey:@"usrturename"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if(memberArray.count == self.selectedItems.count)
        {
            if ([self.navigationItem.rightBarButtonItem.title isEqualToString:@"全选"])
            {
                self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitleStr:@"取消全选" style:UIBarButtonItemStylePlain target:self action:@selector(selectAll:)];
