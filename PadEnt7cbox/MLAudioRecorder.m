    ...
    if (inNumPackets > 0) {
        ...
        if (pcmData&&pcmData.length>0) {
            ...
            dispatch_async(recorder.writeFileQueue, ^{
                ...
                if(recorder.oFileWriteDelegate&&![recorder.oFileWriteDelegate writeIntoFileWithData:pcmData withRecorder:recorder inAQ:inAQ inStartTime:inStartTime inNumPackets:inNumPackets inPacketDesc:inPacketDesc]){
                    //保证只处理了一次
                    if (dispatch_semaphore_wait(recorder.semError,DISPATCH_TIME_NOW)==0){
                        //回到主线程
                        dispatch_async(dispatch_get_main_queue(),^{
...
- (void)startRecording
{
    ...
    if(!self.oFileWriteDelegate||![self.oFileWriteDelegate respondsToSelector:@selector(createFileWithRecorder:)]||![self.oFileWriteDelegate respondsToSelector:@selector(writeIntoFileWithData:withRecorder:inAQ:inStartTime:inNumPackets:inPacketDesc:)]||![self.oFileWriteDelegate respondsToSelector:@selector(completeWriteWithRecorder:withIsError:)]){
        [self postAErrorWithErrorCode:MLAudioRecorderErrorCodeAboutOther andDescription:@"OFileWriterDelegate的代理未设置或其代理方法不完整"];
        return;
    }
    ...
    dispatch_sync(self.writeFileQueue, ^{
        ...
        if (self.oFileWriteDelegate&&![self.oFileWriteDelegate createFileWithRecorder:self]) {
            dispatch_async(dispatch_get_main_queue(),^{
                [self postAErrorWithErrorCode:MLAudioRecorderErrorCodeAboutFile andDescription:@"为音频输入建立文件失败"];
            });
            isContinue = NO;
...
- (void)stopRecording
{
    ...
    if (self.isRecording) {
        ...
        dispatch_sync(self.writeFileQueue, ^{
            ...
            if (self.oFileWriteDelegate&&![self.oFileWriteDelegate completeWriteWithRecorder:self withIsError:NO]) {
                dispatch_async(dispatch_get_main_queue(),^{
                    [self postAErrorWithErrorCode:MLAudioRecorderErrorCodeAboutFile andDescription:@"为音频输入关闭文件失败"];
                });
                isContinue = NO;
