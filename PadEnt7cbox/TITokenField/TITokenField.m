...
- (void)setup {
    ...
    self.titileField=[[UITextField alloc] initWithFrame:CGRectMake(10, tokenFieldBottom+1,self.bounds.size.width-20, 42)];
    [self addSubview:self.titileField];
    ...
    [_separator2 setBackgroundColor:[UIColor colorWithWhite:0.7 alpha:1]];
    ...
    [_contentView2 setBackgroundColor:[UIColor clearColor]];
...
- (void)setFrame:(CGRect)frame {
    ...
    [_contentView setFrame:((CGRect){_contentView.frame.origin, {width, _contentView.bounds.size.height}})];
	[_contentView2 setFrame:((CGRect){_contentView2.frame.origin, {width, _contentView2.bounds.size.height}})];
...
- (void)tokenFieldFrameWillChange:(TITokenField *)field {
    ...
    CGFloat titleFieldBottom=CGRectGetMaxY(_titileField.frame);
    [_separator2 setFrame:((CGRect){{_separator2.frame.origin.x,(titleFieldBottom)},_separator2.bounds.size})];
	[_contentView setFrame:((CGRect){{_contentView.frame.origin.x, (titleFieldBottom + 1)}, _contentView.bounds.size})];
    CGFloat contentViewBottom=CGRectGetMaxY(_contentView.frame);
    [_contentView2 setFrame:((CGRect){{_contentView.frame.origin.x, contentViewBottom}, _contentView2.bounds.size})];
}
...
- (void)setPromptText:(NSString *)text {
	...
	if (text){
		...
		UILabel * label = (UILabel *)_titileField.leftView;
		if (!label || ![label isKindOfClass:[UILabel class]]){
			label = [[UILabel alloc] initWithFrame:CGRectZero];
			[label setTextColor:[UIColor colorWithWhite:0.5 alpha:1]];
			[_titileField setLeftView:label];
            
			[_titileField setLeftViewMode:UITextFieldViewModeAlways];
		}
		
		[label setText:text];
		[label setFont:[UIFont systemFontOfSize:(_tokenField.font.pointSize + 1)]];
		[label sizeToFit];
	}
	else
	{
		[_titileField setLeftView:nil];
	}
}
@end
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
			...
			if (_tokens.count > 1 && untokSize.width > availableWidth){
				untokenized = [NSString stringWithFormat:@"%d 个收件人", titles.count];
			}
